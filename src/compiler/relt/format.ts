...
export function formatRelt(x: ReltModule | ReltDefinition | ReltExpression | ReltType | ReltModelModifier): string {
  ...
  const helper = (x: ReltModule | ReltDefinition | ReltExpression | ReltType | ReltModelModifier | ReltObjectProperty): string => {
    return match(x)
      ...
      .with({ kind: "ReltWhereExpression" }, x => `${x.head === undefined ? "" : helper(x.head)}where ${helper(x.condition)}`)
      .with({ kind: "ReltSortExpression" }, x => `${x.head === undefined ? "" : helper(x.head)}sort ${x.columns.map(helper).join(', ')} ${x.op}`)
      .with({ kind: "ReltOverExpression" }, x => `${x.head === undefined ? "" : helper(x.head)}over ${helper(x.column)}`)
      .with({ kind: "ReltJoinExpression" }, x => `${x.head === undefined ? "" : helper(x.head)}${x.op} join${x.on === undefined ? "" : ` ${helper(x.on)}`} ${helper(x.other)}`)
      .with({ kind: "ReltUnionExpression" }, x => `${x.head === undefined ? "" : helper(x.head)}union ${helper(x.other)}`)
      .with({ kind: "ReltWithExpression" }, x => `${x.head === undefined ? "" : helper(x.head)}with {${x.properties.map(x => `${helper(x)},\n`)}}`)
